...
# ==============================================================================
"""Evalute the word error rate (WER) for ASR tasks.
...
def GenerateSummaryFromErrs(nref, errs):
  ...
  return str_sum, str_details, wer
...
def ComputeWER(hyp, ref, diagnosis=False):
  ...
  nref = max(len(rs), 1)
  total_errors = sum(errs.values())
  return errs, nref, aligned_html, total_errors
...
def AverageWERs(hyps, refs, verbose=True, diagnosis=False):
  ...
  for hyp, ref in zip(hyps, refs):
    errs_i, nref_i, diag_str, x = ComputeWER(hyp, ref, diagnosis)
    if diagnosis:
